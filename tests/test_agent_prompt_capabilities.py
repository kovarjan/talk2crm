...
CRM_TOOLS = {"rag_search_tool", "crm_query_tool", "crm_record_detail_tool", "my_meetings_tool", "crm_action_tool", "get_company_overview"}
...
def test_prompt_lists_only_present_tools_in_order() -> None:
    ...
    assert "6. get_company_overview(" in prompt
...
def test_prompt_numbers_web_tools_after_crm_tools() -> None:
    ...
    assert "7. web_search_tool(" in prompt
    assert "8. web_fetch_tool(" in prompt
...
def test_form_capability_adds_block_and_tools() -> None:
    ...
    assert "7. propose_form_fields_tool(" in prompt
...
def test_aggregate_blocks_follow_registry_tools() -> None:
    ...
    assert "7. crm_aggregate_tool(" in prompt
    assert "8. math_tool(" in prompt
